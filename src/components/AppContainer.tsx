...
import { BarChart3, LogOut as LogOutIcon, User, Mail, ArrowLeft } from "lucide-react";
...
useEffect(() => {
  if (!user) return;
  const classesCollectionRef = collection(db, "classespordia", user.email ?? "noemail", "dates");
  getDocs(classesCollectionRef).then((querySnapshot) => {
    let newCalendar = calendario;
    querySnapshot.docs.forEach((doc) => {
      const data = doc.data();
      newCalendar = newCalendar.setClassCounter(new Date(data.date), data.count);
    });
  ...
  const handleSaveVacations = async (vacationData: {
    ...
    const updatedVacationNumber: Record<string, any> = {};
    updatedVacationNumber[`${key}-vacationNumber`] = vacationData.vacationNumber;
  ...
  if (showOnlyChart) {
    ...
    const diasDelMes = new Date(currentDate.getFullYear(), currentDate.getMonth() + 1, 0).getDate();
      ...
      <div className="min-h-screen w-screen bg-gradient-to-b from-gray-50 to-emerald-50 text-gray-800 overflow-y-auto px-6 sm:px-12 py-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-3xl font-semibold">Gráfica de clases – {nombreMes}</h1>
          </div>
          <div className="flex flex-row flex-wrap items-center justify-end gap-3 mt-2 sm:mt-0">
            <Button
              ...
              sx={{ textTransform: "none", borderRadius: 4, fontWeight: 500, height: "40px", minWidth: "180px" }}
            >
  ...
  const LogOut = () => (
    <Button
      ...
      sx={{ borderRadius: 4, textTransform: "none", fontWeight: 500, height: "38px", padding: "0 12px", marginTop: 0 }}
    >
      Cerrar sesión
    </Button>
  );
  ...
  return (
    <div className={`w-screen bg-gradient-to-b from-white to-emerald-50 text-gray-800 overflow-y-auto ${isMobile ? "px-4 pt-6 pb-10 space-y-4" : "px-12 pt-6 pb-10 space-y-6"}`}>
      <header className={`flex flex-col items-center relative text-center ${isMobile ? "mb-4 space-y-3" : "mb-8 space-y-6"}`}>
        <div className={`flex w-full items-center justify-between ${isMobile ? "flex-col gap-3 text-sm" : "flex-row px-8 mt-2 text-base"}`}>
        ...
        <img src={calendar} alt="Logo calendario" className={`w-full object-contain mt-0 ${isMobile ? "max-w-[140px]" : "max-w-[230px] -translate-x-14"}`} />
      </header>

      <section className={`bg-white rounded-xl shadow-md border border-gray-200 w-full ${isMobile ? "p-0" : "p-6"}`}>
        <Month
          ...
          setClassCount={async (day, count) => {
            ...
            const docRef = doc(db, "classespordia", email, "dates", day.toISOString().split("T")[0]);
            await setDoc(docRef, { date: day.toISOString(), count }, { merge: true });
          }}
      ...
      <div className="flex justify-center mt-8">
        <Button
          ...
          onClick={() => navigate(`/admin/grafica?year=${currentDate.getFullYear()}&month=${currentDate.getMonth()}`)}
          sx={{ borderRadius: 4, textTransform: "none", fontWeight: 500, px: 3, py: 1 }}
        >
