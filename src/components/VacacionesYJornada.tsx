...
import { Briefcase, Plane, Calendar, X, AlertCircle } from "lucide-react";
...
const VacacionesYJornada: React.FC<VacacionesYJornadaProps> = ({
  ...
  const holidays = currentYear === 2026 ? spanishHolidays2026 : spanishHolidays2025;
  ...
  const cardBase = "bg-emerald-50 border border-emerald-200 rounded-md p-4 shadow-sm flex flex-col text-sm";
        ...
        <div className="bg-white rounded-md border border-gray-200 p-3 text-sm space-y-2">
          <div className="flex justify-between items-center">
            <span className="flex items-center gap-1 text-gray-700">
              <Briefcase size={15} className="text-emerald-600" /> Tipo de jornada:
            </span>
            <select
              value={jornada}
              onChange={(e) => setJornada(e.target.value as "media" | "completa")}
          ...
          <div className="flex justify-between items-center pt-2 border-t border-gray-200">
            ...
            >
              {naturalVacationDays > 0 ? `${naturalVacationDays} días` : "Configurar"}
            </button>
          </div>
        </div>
      </div>
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
  value={naturalDays}
  onChange={(e) => handleNaturalDaysChange(Number(e.target.value))}
  className="w-full px-3 py-2 border border-gray-300 rounded-md"
>
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-md p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="text-sm">
                      <p className="font-medium text-blue-900">Resumen de vacaciones:</p>
                      <ul className="mt-2 space-y-1 text-blue-800">
                        <li>• Fecha fin: {new Date(calculatedEndDate).toLocaleDateString('es-ES')}</li>
